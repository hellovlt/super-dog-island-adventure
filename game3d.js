...
const READ_STORAGE='superdog-read-aloud';// The recorded voice is on from the first launch; the switch is there for turning it off.
let readAloud=(()=>{try{return localStorage.getItem(READ_STORAGE)!=='off';}catch{return true;}})(),speechUntil=0;
...
function launch(){menuMode=false;const opening=inVillage()?null:STORIES.find(st=>st.kind==='world'&&st.level===game.level);if(opening&&readAloud)setTimeout(()=>speakStory(opening),900);$('menu').hidden=true;$('hud').hidden=false;$('pause').hidden=false;$('touch').hidden=!matchMedia('(pointer:coarse), (max-width:760px)').matches;game.start();ui();$('modal').hidden=true;resetInput();yaw=0;pitch=.35;camera.position.set(game.player.x,game.player.y+7,game.player.z+11);camBase.copy(camera.position);canvas.focus();storeSave();toast('WASD — move · Space twice to double jump · Drag the mouse to orbit');}
